...
const Lesson = ({ course, section, lesson, props }) => {
  ...
  const handleSubmit = event => {
    ...
    dispatch(
      updateLesson(props.match.params.id, section.id, lesson.id, changes)
    );
    ...
    <div style={{ margin: "15px 0" }}>
      {!toggleLessonEdit ? (
        <>
          ...
          >
            <TrashCan
              style={{ fontSize: "2.6rem", color: "#414D55" }}
              onClick={handleModalOpen}
            />
            <EditIconButton
              style={{ fontSize: "2.6rem", color: "#414D55" }}
              onClick={handleToggleLessonEdit}
            />
            <span
              style={{
                paddingLeft: "20px",
            ...
            <span style={{ marginRight: "10px" }}>
              {lesson.type === "video" ? (
                <PlayCircleFilledIcon style={{ fontSize: "2.6rem" }} />
              ) : lesson.type === "reading" ? (
                <BookIcon style={{ fontSize: "2.6rem" }} />
              ) : lesson.type === "quiz" ? (
                <AssignmentIcon style={{ fontSize: "2.6rem" }} />
              ) : lesson.type === "assignment" || "discussion" ? (
                <DescriptionIcon style={{ fontSize: "2.6rem" }} />
              ) : null}
            </span>
          </div>
        </>
